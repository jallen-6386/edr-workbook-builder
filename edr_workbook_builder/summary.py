...
a per-sheet inventory table, column inventory matrix, parent/child process
relationship table, analyst notes area, and suggested Excel Copilot prompts.
"""
...
# Each value is a list of column name candidates (case-insensitive match).
...
def create_summary_sheet(
    ...
    row = _kv(ws, row, "Tool:", "EDR Workbook Builder v0.2.0")
    row += 1
    ...
    # v0.2: parent/child relationship table (skipped silently when no PID columns found)
    ...
    # Column widths — must accommodate the widest section (column inventory has the most columns)
    ...
    col_widths = {
        ...
        4: 14,   # Row Count / Child PID
        5: 50,   # Status / Source Sheet
    }
    # Key group columns (centered checkmarks)
    for i in range(key_group_count):
        col_widths[6 + i] = 14
    # All Columns (last column in inventory)
